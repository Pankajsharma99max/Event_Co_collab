...
export function GoogleAuthSection({ callbackUrl }: { callbackUrl: string }) {
  ...
  return (
    <div className="mb-6">
      <button
        type="button"
        onClick={() => signIn("google", { callbackUrl: withBasePath(callbackUrl) })}
        className="flex w-full items-center justify-center gap-2 rounded-full border border-border-strong bg-glass px-4 py-2.5 font-medium text-foreground transition hover:bg-glass-strong"
      >
        <GoogleIcon />
